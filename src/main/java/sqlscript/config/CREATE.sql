...
#
#oauth2 configurations that could be linked from other tables
CREATE TABLE oauth2(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,authendpoint VARCHAR(1024)NOT NULL,authscope VARCHAR(1024)NOT NULL,tokenendpoint VARCHAR(1024)NOT NULL,clientsecret VARCHAR(1024)NOT NULL,clientid VARCHAR(1024)NOT NULL,authcode VARCHAR(8192),accesstoken VARCHAR(8192),refreshtoken VARCHAR(8192),accesstokenuntil BIGINT DEFAULT 0 NOT NULL,username VARCHAR(256),customparamauth VARCHAR(1024),pkce INTEGER DEFAULT 0 NOT NULL,pkceverifier VARCHAR(128) DEFAULT '_new' not NULL,rfc INTEGER DEFAULT 1 NOT NULL,clientcredentialssecret INTEGER DEFAULT 1 NOT NULL);
#
#Create partner table
CREATE TABLE partner(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,as2ident VARCHAR(255),partnername VARCHAR(255),islocal INTEGER,sign INTEGER,encrypt INTEGER,email VARCHAR(255),url VARCHAR(255),mdnurl VARCHAR(255),msgsubject VARCHAR(255),contenttype VARCHAR(255),syncmdn INTEGER,pollignorelist VARCHAR(1024),pollinterval INTEGER,msgcompression INTEGER,signedmdn INTEGER,usehttpauth INTEGER,httpauthuser VARCHAR(256),httpauthpass VARCHAR(256),usehttpauthasyncmdn INTEGER,httpauthuserasnymdn VARCHAR(256),httpauthpassasnymdn VARCHAR(256),keeporiginalfilenameonreceipt INTEGER,partnercomment OBJECT,notifysend INTEGER,notifyreceive INTEGER,notifysendreceive INTEGER,notifysendenabled INTEGER,notifyreceiveenabled INTEGER,notifysendreceiveenabled INTEGER,contenttransferencoding INTEGER,httpversion VARCHAR(3) DEFAULT '1.1' NOT NULL,maxpollfiles INTEGER DEFAULT 100 NOT NULL,partnercontact OBJECT,partneraddress OBJECT,algidentprotatt INTEGER DEFAULT 1 NOT NULL,enabledirpoll INTEGER DEFAULT 1 NOT NULL,overwritelocalsecurity INTEGER DEFAULT 0 NOT NULL,useoauth2message INTEGER DEFAULT 0 NOT NULL,useoauth2mdn INTEGER DEFAULT 0 NOT NULL,oauth2idmessage INTEGER,oauth2idmdn INTEGER,FOREIGN KEY(oauth2idmessage)REFERENCES oauth2(id),FOREIGN KEY(oauth2idmdn)REFERENCES oauth2(id));
CREATE INDEX idx_partner_islocal ON partner(islocal);
CREATE INDEX idx_partner_as2ident ON partner(as2ident);
CREATE INDEX idx_partner_partnername ON partner(partnername);
...
#
#notification
CREATE TABLE notification(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,mailhost VARCHAR(255),mailhostport INTEGER,notificationemailaddress VARCHAR(255),notifycertexpire INTEGER DEFAULT 0 NOT NULL,notifytransactionerror INTEGER DEFAULT 0 NOT NULL,notifycem INTEGER DEFAULT 0 NOT NULL,notifysystemfailure INTEGER DEFAULT 0 NOT NULL,notifypostprocessing INTEGER DEFAULT 0 NOT NULL,notifyclientserver INTEGER DEFAULT 0 NOT NULL,replyto VARCHAR(255),usesmtpauth INTEGER DEFAULT 0 NOT NULL,smtpauthuser VARCHAR(255),smtpauthpass VARCHAR(255),notifyresend INTEGER DEFAULT 0 NOT NULL,security INTEGER DEFAULT 0 NOT NULL,maxnotificationspermin INTEGER DEFAULT 2 NOT NULL,notifyconnectionproblem INTEGER DEFAULT 0 NOT NULL,usesmtpoauth2 INTEGER DEFAULT 0 NOT NULL,smtpoauth2id INTEGER,FOREIGN KEY(smtpoauth2id)REFERENCES oauth2(id));
...
CREATE TABLE keydata(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,storagedata OBJECT NOT NULL,storagetype INTEGER NOT NULL,purpose INTEGER NOT NULL,lastchanged BIGINT DEFAULT 0 NOT NULL,securityprovider VARCHAR(65)DEFAULT 'BC' NOT NULL,UNIQUE(purpose));
