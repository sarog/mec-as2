...
#message log
#
CREATE TABLE messagelog( id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,messageid VARCHAR(255),timestamputc TIMESTAMP,loglevel INTEGER,details OBJECT,FOREIGN KEY(messageid)REFERENCES messages(messageid),sequenceno BIGINT NOT NULL);
...
#High Availability
#
CREATE TABLE highavail(id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,uniqueid VARCHAR(8) NOT NULL,localip VARCHAR(255) NOT NULL,publicip VARCHAR(255),cloudinstanceid VARCHAR(65),hostname VARCHAR(128) NOT NULL,clientcount INTEGER DEFAULT 0 NOT NULL,operationsystem VARCHAR(128) NOT NULL,productversion VARCHAR(128) NOT NULL,starttime BIGINT NOT NULL,lastupdatetime BIGINT NOT NULL);
