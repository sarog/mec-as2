//$Header: /mec_as2/de/mendelson/util/systemevents/SystemEventManagerImplAS2.java 36    15/04/26 12:44 Heller $
...
public class SystemEventManagerImplAS2 extends SystemEventManager {
    ...
    private static final String PRODUCT_NAME = AS2ServerVersion.getProductName();
    ...
    public void newEventResendDetected(AS2MessageInfo newMessageInfo, 
            AS2MessageInfo alreadyExistingMessageInfo,
        ...
        replacement.setProperty("${PRODUCTNAME}", PRODUCT_NAME);
        replacement.setProperty("${HOST}", this.getHostname());
        replacement.setProperty("${EXISTING_MESSAGE_INIT_TIME}", 
                alreadyExistingMessageInfo.getInitDate().toString());
        ...
        SystemEvent event = new SystemEvent(SystemEvent.Severity.WARNING,
                ...
                SystemEvent.Type.TRANSACTION_REJECTED_RESEND);
    ...
    public void newEventPostprocessingError(String errorText, String messageId,
            Partner sender, Partner receiver, ProcessingEventType processType, 
            ProcessingEventTriggerType triggerType) throws Exception {
        String template = "template_notification_postprocessing_error";
        Properties replacement = new Properties();
        replacement.setProperty("${PRODUCTNAME}", PRODUCT_NAME);
        replacement.setProperty("${HOST}", this.getHostname());
        replacement.setProperty("${EVENT_TYPE}", ProcessingEvent.getLocalizedEventType(triggerType));
    ...
    public void newEventSSLCertificateAddedByCEM(Partner partner, KeystoreCertificate cert) throws Exception {
        ...
        SystemEvent event = new SystemEvent(SystemEvent.Severity.INFO, 
                SystemEvent.Origin.TRANSACTION, SystemEvent.Type.CERTIFICATE_EXCHANGE_ANY);
        event.readFromNotificationTemplate(template, replacement);
        this.storeEventToFile(event);
    }
     ...
     * connector has to be restarted before these changes are taken
     */
    public void newEventEncSignCertificateAddedByCEM(Partner partner, 
            KeystoreCertificate cert) throws Exception {
        ...
        SystemEvent event = new SystemEvent(SystemEvent.Severity.INFO, 
                SystemEvent.Origin.TRANSACTION, 
                SystemEvent.Type.CERTIFICATE_EXCHANGE_ANY);
        event.readFromNotificationTemplate(template, replacement);
        this.storeEventToFile(event);
    }
    ...
    public void newEventPartnerSendQuotaExceededCheck(Partner localStation, Partner partner,
        ...
        if (partner.isNotifySendEnabled() && partner.getNotifySend() == entry.getSendMessageCount()) {
            ...
            SystemEvent event = new SystemEvent(SystemEvent.Severity.WARNING, SystemEvent.Origin.SYSTEM,
                    SystemEvent.Type.QUOTA_SEND_EXCEEDED);
            try {
                event.readFromNotificationTemplate(template, replacement);
                this.storeEventToFile(event);
            } catch (Exception e) {
    ...
    public void newEventPartnerReceiveQuotaExceededCheck(Partner localStation, Partner partner,
        ...
        if (partner.isNotifyReceiveEnabled() && partner.getNotifyReceive() == entry.getReceivedMessageCount()) {
            ...
            SystemEvent event = new SystemEvent(SystemEvent.Severity.WARNING, SystemEvent.Origin.SYSTEM,
                    SystemEvent.Type.QUOTA_RECEIVE_EXCEEDED);
            try {
                event.readFromNotificationTemplate(template, replacement);
                this.storeEventToFile(event);
            } catch (Exception e) {
    ...
    public void newEventConnectionProblem(Partner receiver, AS2Info as2Info, String errorMessage, String hint) {
        ...
        replacement.setProperty("${MESSAGE}", errorMessage == null ? "[NONE]" : errorMessage);
        replacement.setProperty("${MESSAGE_ID}", as2Info.getMessageId());
        replacement.setProperty("${HINT}", hint);
        SystemEvent event = new SystemEvent(SystemEvent.Severity.ERROR, SystemEvent.Origin.TRANSACTION,
                SystemEvent.Type.CONNECTIVITY_ANY);
        try {
            event.readFromNotificationTemplate(template, replacement);
            this.storeEventToFile(event);
        } catch (Exception e) {
     ...
     */
    @Override
    public void newEvent(SystemEvent.Severity severity, 
            SystemEvent.Origin origin, SystemEvent.Type type, String subject, String body) {
    ...
    public void systemFailure(Throwable exception, 
        ...
        try {
            ...
            if (exception instanceof SQLException && statement != null 
                    && statement instanceof DebuggablePreparedStatement) {
                builder.append("\n");
     ...
     */
    @Override
    public void systemFailure(Throwable exception, SystemEvent.Type eventType) {
    ...
    @Override
    public void systemFailure(Throwable exception) {
        systemFailure(exception, SystemEvent.Type.OTHER, null);
    }
    ...
    public void newEventPartnerSendReceiveQuotaExceededCheck(Partner localStation, Partner partner,
        ...
        if (partner.isNotifySendReceiveEnabled() && partner.getNotifySendReceive() == entry.getSendMessageCount() + entry.getReceivedMessageCount()) {
            ...
            replacement.setProperty("${QUOTA}", String.valueOf(partner.getNotifyReceive()));
            SystemEvent event = new SystemEvent(SystemEvent.Severity.WARNING, SystemEvent.Origin.SYSTEM,
                    SystemEvent.Type.QUOTA_SEND_RECEIVE_EXCEEDED);
            try {
                event.readFromNotificationTemplate(template, replacement);
                this.storeEventToFile(event);
            } catch (Exception e) {
     ...
     */
    public void newEventCertificateWillExpire(KeystoreCertificate certificate, 
            int expireDuration) throws Exception {
        ...
        SystemEvent event = new SystemEvent(SystemEvent.Severity.INFO, SystemEvent.Origin.SYSTEM, SystemEvent.Type.CERTIFICATE_EXPIRE);
    ...
    public void newEventLicenseWillExpire(long expireInDays, String formattedExpireDate) throws Exception {
        ...
        SystemEvent event = new SystemEvent(SystemEvent.Severity.INFO, SystemEvent.Origin.SYSTEM,
                SystemEvent.Type.LICENSE_EXPIRE);
        event.readFromNotificationTemplate(templateName, replacement);
        this.storeEventToFile(event);
    }
    ...
    public void newEventCEMRequestReceived(Partner initiator, String requestId) throws Exception {
        ...
        SystemEvent event = new SystemEvent(SystemEvent.Severity.INFO,
                SystemEvent.Origin.TRANSACTION,
                SystemEvent.Type.CERTIFICATE_EXCHANGE_REQUEST_RECEIVED);
        event.readFromNotificationTemplate(templateName, replacement);
        this.storeEventToFile(event);
    }
     ...
     */
    public void newEventCertificateChangeShouldHappenNowByCEM(CertificateManager manager, Partner partner, 
            CEMEntry.Category category) throws Exception {
        ...
        String description = partner.getPartnerCertificateInformationList().getCertificatePurposeDescription(
                manager, partner, PartnerCertificateInformation.Category.of(category.toInt()));
        replacement.setProperty("${CERTIFICATEDESCRIPTION}", description);
        StringBuilder techInfo = new StringBuilder();
        PartnerCertificateInformation info = partner.getCertificateInformation(
                PartnerCertificateInformation.Category.of(category.toInt()));
        if (info != null) {
    ...
    public void newEventTransactionError(String messageId, IDBDriverManager dbDriverManager) {
        ...
        try {
            ...
            if (info.getSenderId() != null) {
                Partner sender = partnerAccess.getPartnerByAS2Id(info.getSenderId());
                if (sender != null) {
                    senderName = sender.getName();
                }
            }
            if (info.getReceiverId() != null) {
                Partner receiver = partnerAccess.getPartnerByAS2Id(info.getReceiverId());
                if (receiver != null) {
                    receiverName = receiver.getName();
                }
            }
            ...
            Date dateWrapper = new Date();
            for (LogEntry entry : entries) {
                if (log.length() > 0) {
                    log.append("\n");
                }
                dateWrapper.setTime(entry.getMillis());
            ...
            SystemEvent event = new SystemEvent(SystemEvent.Severity.ERROR, 
                    SystemEvent.Origin.TRANSACTION, 
                    SystemEvent.Type.TRANSACTION_ERROR);
            event.readFromNotificationTemplate(templateName, replacement);
            this.storeEventToFile(event);
        } catch (Exception e) {
            e.printStackTrace();
